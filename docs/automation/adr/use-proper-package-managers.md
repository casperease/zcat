...
- **Remove Chocolatey from target machines.** `Uninstall-Chocolatey` removes the Chocolatey installation, its environment variables, and its PATH entries. It is idempotent — safe to run when Chocolatey is not present. `Install-WorkstationTools` calls it as its first step so that environments previously provisioned with Chocolatey are cleaned up automatically.

- **`Get-WorkstationToolsStatus` reports Chocolatey as unwanted.** If `choco` is on PATH, the status output includes a Chocolatey entry with status `Unwanted` and the recommended action `Run Uninstall-Chocolatey`.
...
- Machines that previously used Chocolatey are cleaned up automatically by `Install-WorkstationTools`. No manual intervention required.
