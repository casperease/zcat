Describe 'Assert-MetaConfiguration' {
    ...
    It 'passes for current meta.yml' {
        ...
        { & (Get-Module Zcat.Meta) { Assert-MetaConfiguration $args[0] } $config } | Should -Not -Throw
    }

    It 'passes for minimal valid config' {
        { & (Get-Module Zcat.Meta) { Assert-MetaConfiguration $args[0] } (Copy-Object $baseConfig) } | Should -Not -Throw
    }

    It 'throws when missing required top-level key' {
        $bad = [ordered]@{ subscription_types = @('prod') }
        { & (Get-Module Zcat.Meta) { Assert-MetaConfiguration $args[0] } $bad } | Should -Throw '*Missing required*'
    }
    ...
    It 'throws when customer references unknown environment_type' {
        ...
        { & (Get-Module Zcat.Meta) { Assert-MetaConfiguration $args[0] } $bad } | Should -Throw '*unknown environment_type*'
    }

    It 'throws for duplicate subscription_types' {
        $bad = Copy-Object $baseConfig
        $bad.subscription_types = @('prod', 'prod')
        { & (Get-Module Zcat.Meta) { Assert-MetaConfiguration $args[0] } $bad } | Should -Throw '*Duplicate subscription_type*'
    }
    ...
    It 'throws when environment_type appears in both customer and shared' {
        ...
        { & (Get-Module Zcat.Meta) { Assert-MetaConfiguration $args[0] } $bad } | Should -Throw '*both customer and shared*'
    }
    ...
    It 'throws when environment has invalid subscription_type' {
        ...
        { & (Get-Module Zcat.Meta) { Assert-MetaConfiguration $args[0] } $bad } | Should -Throw '*invalid subscription_type*'
    }
}
