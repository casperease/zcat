...
| **Symlinks** (`Documents\PowerShell` → local dir)                             | Technically works — the symlink redirects `$PROFILE`, config, and modules to a local path. However, it requires migrating existing content out of `Documents\PowerShell` on DFS before the symlink can be created (the directory must not exist). Users will have profiles, module caches, and history files there. Automating this migration safely across all users is too complex and error-prone to justify. Also requires `Bypass` execution policy — profiles loaded through a symlink from a DFS-origin path are treated as "remote" by `RemoteSigned` (same as files directly on DFS). |
