Describe 'Remove-PermanentPath' {
    BeforeAll {
        $script:tempDir = Join-Path ([System.IO.Path]::GetTempPath()) "zcap-test-path-$([guid]::NewGuid().ToString('N').Substring(0,8))"
    ...
    if (-not $IsWindows) {
        Context 'Unix persistence' {
            BeforeAll {
                $script:testProfile = Join-Path ([System.IO.Path]::GetTempPath()) "zcap-test-profile-$([guid]::NewGuid().ToString('N').Substring(0,8)).ps1"
            }
            ...
            It 'removes marker block from profile' {
                ...
                try {
                    ...
                    $content | Should -Match '>>> zcap PATH TestTool >>>'

                    # Act: remove it
                    Remove-PermanentPath $tempDir -Label 'TestTool'
                    $content = Get-Content $testProfile -Raw
                    $content | Should -Not -Match '>>> zcap PATH TestTool >>>'
                }
                finally {
