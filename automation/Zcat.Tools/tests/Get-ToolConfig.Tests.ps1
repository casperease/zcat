Describe 'Get-ToolConfig' {
    ...
    Context 'Get-ToolConfig behavior' {
        It 'returns config for <Tool>' -ForEach $script:toolEntries {
            $script:result = & (Get-Module Zcat.Tools) { Get-ToolConfig -Tool $args[0] } $Tool
        ...
        It 'throws for unknown tool' {
            { & (Get-Module Zcat.Tools) { Get-ToolConfig -Tool 'FakeTool' } } | Should -Throw '*Unknown tool*'
        }

        It 'caches across calls' {
            $script:first = & (Get-Module Zcat.Tools) { Get-ToolConfig -Tool 'Python' }
            $script:second = & (Get-Module Zcat.Tools) { Get-ToolConfig -Tool 'Python' }
